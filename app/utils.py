...
def humanize_time(dt):
    ...
    if years > 0:
        ...
        return "الآن"        
def process_single_item_data(item_data):
    """معالجة بيانات عنصر واحد فقط"""
    # معالجة الصور
    main_image = ''
    thumbnail_url = item_data.get('product_thumbnail') or item_data.get('thumbnail')
    if thumbnail_url and isinstance(thumbnail_url, str):
        main_image = thumbnail_url
    else:
    ...
    item_barcode = generate_item_barcode("temp", item_id) if item_id else None
    ...
    return {
        ...
        'price': item_data.get('amounts', {}).get('price_without_tax', {}).get('amount', 0),
        'currency': item_data.get('currency', 'SAR'),
        'total_price': item_data.get('amounts', {}).get('total', {}).get('amount', 0),
