    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if user and user.check_password(password):
                ...
                response.set_cookie('user_id', str(user.id), max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
                ...
                if user.salla_access_token:
                    response.set_cookie('salla_access_token', user.get_access_token(), max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
                    response.set_cookie('salla_refresh_token', user.salla_refresh_token, max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
            ...
            if employee and employee.check_password(password):
                ...
                response = make_response(redirect(url_for('dashboard.index')))
                response.set_cookie('user_id', str(employee.id), max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
                ...
                if store_admin and store_admin.salla_access_token:
                    response.set_cookie('salla_access_token', store_admin.get_access_token(), max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
                    response.set_cookie('salla_refresh_token', store_admin.get_refresh_token(), max_age=timedelta(days=14).total_seconds(), httponly=True, secure=False)
