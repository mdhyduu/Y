...
def handle_status_transitions(order_id, new_status_type, custom_status_id=None):
    ...
    try:
        ...
        # --- حذف الحالات من OrderEmployeeStatus ---
        ...
        for status in employee_statuses:
            ...
            if (not is_custom and cs.name in DEFAULT_NAMES) or (is_custom and cs.name not in DEFAULT_NAMES):
                removed_statuses.append(cs.name)
                db.session.delete(status)

        # --- حذف الحالات من OrderStatusNote ---
        status_notes = OrderStatusNote.query.filter_by(order_id=str(order_id)).all()
        for note in status_notes:
            if (not is_custom and note.status_flag in DEFAULT_NAMES) or (is_custom and note.status_flag not in DEFAULT_NAMES):
                removed_statuses.append(note.status_flag)
                db.session.delete(note)

        # --- تسجيل ملاحظات الانتقال ---
