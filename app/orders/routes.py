...
def order_details(order_id):
    ...
    # إصلاح مشكلة تحميل employee مع العلاقات
    if current_employee:
    ...
    try:
        ...
        def fetch_order_data():
            ...
            url = f"{Config.SALLA_ORDERS_API}/{order_id}"
        ...
        def fetch_order_items():
            ...
            url = f"{Config.SALLA_BASE_URL}/orders/items"
            response = requests.get(url, headers=headers, params={'order_id': order_id, 'include': 'images'}, timeout=15)
            response.raise_for_status()
            return response.json().get('data', [])
        ...
        # ✅ جلب العنوان من قاعدة البيانات فقط
