...
@orders_bp.route('/')
def index():
    """عرض قائمة الطلبات (سلة + مخصصة) مع آخر حالة فقط"""
    ...
    try:
        ...
        if search_query:
            ...
            salla_query = salla_query.filter(or_(
        ...
        if date_from or date_to:
            if date_from:
                ...
                salla_query = salla_query.filter(SallaOrder.created_at >= df)
                custom_query = custom_query.filter(CustomOrder.created_at >= df)
            if date_to:
                dt = datetime.strptime(date_to, "%Y-%m-%d") + timedelta(days=1)
                salla_query = salla_query.filter(SallaOrder.created_at <= dt)
                custom_query = custom_query.filter(CustomOrder.created_at <= dt)
        ...
        if order_type == "salla":
            ...
            salla_pagination = salla_query.order_by(SallaOrder.created_at.desc()).paginate(page=1, per_page=per_page*2, error_out=False)
            custom_pagination = custom_query.order_by(CustomOrder.created_at.desc()).paginate(page=1, per_page=per_page*2, error_out=False)
