...
def order_details(order_id):
    """عرض تفاصيل الطلب مع المنتجات من سلة (مبسطة بدون shipments)"""
    ...
    try:
        ...
        # ========== العنوان ==========
        address_data = order_data.get("shipping", {}).get("address", {})
        if not address_data:
        ...
        full_address = "، ".join([str(v) for v in address_data.values() if v])
        ...
        product_statuses = {s.product_id: {"status": s.status, "notes": s.notes} for s in OrderProductStatus.query.filter_by(order_id=str(order_id)).all()}
