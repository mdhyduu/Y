...
def assign_orders():
    ...
    # هنا orders = [{id: "12345", type: "salla"}, {id: 7, type: "custom"}]
    ...
    try:
        ...
        for order in orders_data:
            ...
            # تحقق من وجود الطلب
            if order_type == 'salla':
                existing_order = SallaOrder.query.get(str(order_id))
                if not existing_order:
                    continue
                # هل يوجد إسناد قديم؟
                existing_assignment = OrderAssignment.query.filter_by(order_id=order_id, employee_id=employee_id).first()
                if not existing_assignment:
                    db.session.add(OrderAssignment(order_id=order_id, employee_id=employee_id))
                    assigned += 1
                ...
                if not existing_order:
                    continue
                existing_assignment = OrderAssignment.query.filter_by(custom_order_id=order_id, employee_id=employee_id).first()
                if not existing_assignment:
                    db.session.add(OrderAssignment(custom_order_id=order_id, employee_id=employee_id))
                    assigned += 1
        ...
        return jsonify({
            'success': True,
            'message': f'تم إسناد {assigned} طلب إلى الموظف المحدد'
        })
        ...
        return jsonify({
            'success': False,
            'error': f'خطأ أثناء الإسناد: {str(e)}'
        }), 500
