...
def update_product_status(order_id, product_id):
    ...
    current_app.logger.info(f"Received product status update request: order_id={order_id}, product_id={product_id}")
    ...
    try:
        current_app.logger.info(f"Updating product status: order={order_id}, product={product_id}, status={new_status}")
        
        status_obj = OrderProductStatus.query.filter_by(
            order_id=str(order_id),  # تأكد من أن هذا يتطابق مع نوع البيانات في DB
            product_id=str(product_id)
        ).first()
